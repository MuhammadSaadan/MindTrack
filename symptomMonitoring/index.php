...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $symptom = $_POST['symptom']; // Assuming the mood is received from the form input
    ...
    if (empty($user_id) || empty($symptom) || empty($description) || empty($rating)) {
        echo "All fields are required."; // Handle any errors
    } else {
        // Prepared statement to prevent SQL injection
        $sql = "INSERT INTO log_symptoms (user_id, symptom, description, rating) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);

        if ($stmt) {
            $stmt->bind_param("isss", $user_id, $symptom, $description, $rating);
            if ($stmt->execute()) {
...
<ol class="flex text-primary font-semibold dark:text-white-dark">
    ...
    <li class="bg-[#ebedf2] dark:bg-[#1b2e4b]"><a class="bg-primary text-white-light p-1.5 ltr:pl-6 rtl:pr-6 ltr:pr-2 rtl:pl-2 relative  h-full flex items-center before:absolute ltr:before:-right-[15px] rtl:before:-left-[15px] rtl:before:rotate-180 before:inset-y-0 before:m-auto before:w-0 before:h-0 before:border-[16px] before:border-l-[15px] before:border-r-0 before:border-t-transparent before:border-b-transparent before:border-l-primary before:z-[1]">Symptom Monitoring</a></li>

</ol>
<br>

    <div x-data="charts">
        <div class="prose bg-[#f1f2f3] px-6 py-10 sm:px-10 sm:py-16 rounded max-w-full dark:bg-[#1b2e4b] dark:text-white-light">
            <h2 class="text-4xl md:text-5xl mb-4 mt-6 text-center dark:text-white-light">Symptom Monitoring</h2>
            <hr class="my-4 dark:border-[#191e3a]">
            <p class="text-center text-lg text-gray-600 dark:text-gray-400">Log and monitor your symptoms.</p>
        </div>
        ...
        <form method="post" action="/symptomMonitoring/index.php" id="logSymptomForm">

                <label for="ctnSelect1" class="block text-sm font-semibold text-gray-600 dark:text-gray-400 mt-1 mb-2">Symptoms:</label>
                <select name="symptom" id="ctnSelect1" class="form-select text-white-dark" required>
                ...
                <label for="description" class="block text-sm font-semibold text-gray-600 dark:text-gray-400 mt-4 mb-2">Describe your symptom:</label>
                <input type="text" name="description" placeholder="Description" class="form-input" required />
                <label for="rating" class="block text-sm font-semibold text-gray-600 dark:text-gray-400 mt-4 mb-2">Symptom Intensity Level:</label>
                <input type="range" name="rating" class="w-full py-2.5" min="1" max="5" required oninput="updateLevel(this.value)">
<div id="rating-tooltip" class="hidden absolute bg-gray-800 text-white px-2 py-1 rounded text-sm">3</div>
<p class="text-sm text-gray-500 dark:text-gray-400">1: Very Low - 2: Low - 3: Moderate - 4: High - 5: Very High</p>
        ...
        <a href="/symptomMonitoring/view.php" class="btn btn-secondary">View Logged Symptoms</a>
