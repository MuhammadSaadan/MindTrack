...
// Declare $stmt outside the condition
$stmt = null;
    ...
    if (!empty($counselor_picture['tmp_name'])) {
        ...
        if ($isValid) {
            ...
            if (move_uploaded_file($counselor_picture['tmp_name'], $targetFilePath)) {
                ...
                $sql = "INSERT INTO counselors (name, description, email, phone_number, password, picture_path, specialization, languages, experience, education) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($sql);

                if ($stmt) {
                    $stmt->bind_param("ssssssssss", $counselor_name, $counselor_description, $counselor_email, $counselor_phone, $password, $targetFilePath, $counselor_specialization, $counselor_languages, $counselor_experience, $counselor_education);
                    if ($stmt->execute()) {
                        echo "Counselor registration successful!";
                        $_SESSION['counselor_registration_success'] = true;
                        header("Location: ../counsellorAdmin/index.php");
                        exit();
                    } else {
...
// Close $stmt only if it is not null
if ($stmt !== null) {
    $stmt->close();
}
